...
import { AccountConfirmationProps, MutationKeys, MutationOptions } from "@types";

export function useAccountConfirmation(options?: MutationOptions<void>) {
  const { accountConfirmation } = AuthService()

  const { isPending, mutateAsync } = useMutation<void, ResponseErrorProps, AccountConfirmationProps>({
